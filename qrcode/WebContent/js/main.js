...
function Overallrating(rating) {
	...
	$(".star").each(function(index) {
		if ($(this).children().slice(1, 2).children().hasClass("icon-checked")) {
			
			//$(this).children().slice(1, 2).hide("fast");
			$(this).children().slice(1, 2).children().removeClass("icon-checked");
			$(this).children().slice(1, 2).animate({left : '+=10%'},10);
			
		}
	});
	
	
		/*if($(rating).children().slice(1, 2).hasClass("checkboxes")){
			$(rating).children().slice(1, 2).show("fast");
			$(rating).children().slice(1, 2).animate({ left : '-=10%'},250);
		}
		else{*/
	$(rating).children().slice(1, 2).children().addClass('icon-checked');
	//$(rating).children().slice(2, 1).addClass("font-size icon-checked");
	$(rating).children().slice(1, 2).animate({ left : '-=10%'},250);
	//}
